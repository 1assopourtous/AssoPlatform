...
async function signJWT(payload, secret) {
  ...
  const headerEncoded = encodeBase64Url2(encoder.encode(JSON.stringify(header)));
  const payloadEncoded = encodeBase64Url2(encoder.encode(JSON.stringify(payload)));
...
var DB = {
  ...
  async createUser(db, user) {
    await db.prepare("INSERT INTO users (id, email, password_hash, role) VALUES (?, ?, ?, ?)").bind(user.id, user.email, user.passwordHash, user.role).run();
  }
};

// src/auth.ts
var JWT_SECRET = "supersecret";
